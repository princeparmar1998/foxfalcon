...
export default function ProductDetailClient({ 
  ...
  // Synchronize dynamic server page props with client states
  useEffect(() => {
    if (initialProduct) {
    ...
    setLoading(false);
  }, [params.id, initialProduct, initialReviews, initialRecommendations]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          {images.length > 1 && (
            <div className="grid grid-cols-3 gap-4">
              {images.map((img: string, index: number) => (
                ...
                >
                  <Image src={img} alt={`${product.name} ${index}`} fill sizes="100px" className="object-cover" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <p className="text-3xl font-black text-primary font-mono">₹{product.price.toFixed(2)}</p>
          </div>
                  ...
                  <Card key={item.id} className="group relative overflow-hidden bg-card border border-border/40 hover:border-primary/50 transition-all duration-300 rounded-2xl flex flex-col p-0 shadow-sm hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="p-4 space-y-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs md:text-sm font-black text-primary font-mono">₹{item.price.toFixed(0)}</span>
                      </div>
                    </div>
                  </Card>
                );
            ...
            <div className="bg-background/95 backdrop-blur-xl border-t border-border shadow-2xl px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-black text-primary font-mono">₹{product ? product.price.toFixed(0) : ""}</p>
                </div>
                <div className="flex gap-2">
                  {isAddedToCart ? (
                    <Button
